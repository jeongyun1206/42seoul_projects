...
/*   Updated: 2023/02/09 14:18:23 by jnho             ###   ########seoul.kr  */
/*                                                                            */
/* ************************************************************************** */
...
void    unlink_here_doc_files(t_file *input_file_list)
{
...
void    close_all_fds(t_cmd *cmd, int **pipe_arr)
{
    ...
    while (cur_cmd)
    {
        ...
        unlink_here_doc_files(input_file_list);
        if (cur_fds->input_file_fd != -1 && cur_fds->input_file_fd != 0 && cur_fds->input_file_list)
            close(cur_fds->input_file_fd);
        if (cur_cmd->next)
            close(pipe_arr[cur_cmd->pipe_idx][0]);
        if (cur_fds->output_file_fd != -1 && cur_fds->output_file_fd != 1 && cur_fds->output_file_list)
