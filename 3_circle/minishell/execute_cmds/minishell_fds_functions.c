...
/*   Updated: 2023/02/22 17:17:05 by seonghyu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	redirect_input_file(t_fds *fds, t_env *env_list, int pipe_fd)
{
	...
	redirect_set_heredoc(fds->input_file_list, env_list);
	last_in_file = get_last_file(fds->input_file_list);
	if (last_in_file)
		fds->input_file_fd = open(last_in_file->file_name, O_RDONLY);
	if (fds->input_file_fd == -1)
	{
		write(2, last_in_file->file_name, ft_strlen(last_in_file->file_name));
		write(2, ": ", 2);
		perror("");
	}
}
